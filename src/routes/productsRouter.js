...
productsRouter.get("/", async (req, res) => {
    try {
        ...
        res.status(200).send({ status: "success", payload: productArray.docs, totalPages: productArray.totalPages, page: productArray.page, hasPrevPage: productArray.hasPrevPage, hasNextPage: productArray.hasNextPage, prevPage: productArray.prevPage, nextPage:productArray.nextPage, prevLink: productArray.hasPrevPage ? fullUrl+`&page=${page-1}` : null,nextLink: productArray.hasNextPage ? fullUrl+`&page=${Number(page)+1}` : null});
    } catch (error) {
